<template>
        ...
        <v-expansion-panel-content style="background-color:#311B92;">
            ...
            <template>
            <div slot="header">
                <h3 style="position: absolute; top: 0; right: 0; margin: 6px 50px 0 0;">Level: {{LVL_UP}}</h3>
            ...
            <v-card-text>
                ...
                <v-flex xs12 md5>
                    <v-layout row wrap>
                        <!-- <small style="color:orange;position: absolute;top: 0;margin-left:100px">Leveling add stats to team just when creating new post</small> -->
                        <v-flex xs2>
...
export default {
    ...
    data () {
        return {
            ...
            attspeedprecent: this.hero.stats.attackspeed,
            mpregen: this.hero.stats.mpregen,
            hpregen: this.hero.stats.hpregen,
    ...
    created() {
        ...
        // IF HAVE ITEMS, ADD STATS
        if (this.hero.itemSet.length != 0) {
            this.hero.itemSet.forEach(x => {
            this.addStats(x.stats)
        })
        }
        //IF HAVE LEVEL UP, ADD STATS PRE LEVEL
        if(this.hero.lvl > 1) {
            for (let index = 0; index < this.hero.lvl -1; index++) {
                this.addLvLup()
            }
        }
    ...
    computed: {
        ...
        LVL_UP: {
            get: function() { return this.hero.lvl},
            set: function(val) {this.hero.lvl = val}
        },
        Attack_Damage: {
    ...
    methods: {
        ...
        addItem (item, name) {
            if (this.hero.itemSet.length < 6 ) {
             ...
             this.$emit('addToTeamStats', item.stats)
        ...
        addTo (test, value) {
            if(test == test){
                if (test == "Attack_Damage" || test == "Ability_Power") {
                    this[test] += parseInt(value)
                }else if (test == "Attack_Speed") {
                    this[test] += this.attspeedprecent * (parseInt(value) / 100)
                }else if (test == "Critical_Strike_Chance" || test == "Cooldown_Reduction" || test == "Life_Steal") {
                this[test] += parseInt(value)
                }else if (test == "Base_Mana_Regen") {
                    this[test] += this.mpregen * (parseInt(value) / 100)
                }else if (test == "Base_Health_Regen") {
                    this[test] += this.hpregen * (parseInt(value) / 100)
                }else if (test == "Bonus_Health") {
                    this.Health += this.hp * (parseInt(value) / 100)
                }else {
